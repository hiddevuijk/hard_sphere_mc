...
class SystemMC {
 public:
  ...
  unsigned int GetNumberOfParticles() const { return number_of_particles_;}
...
void SystemMC<Potential>::MCMove()
{
  ...
  for (std::list<unsigned int>::iterator it =
                               neighbor_list_[i].begin(); 
       it != neighbor_list_[i].end(); ++it) {

      if (systemMC_helper::distance_squared(
            new_position, positions_[*it],
            system_size_x_, system_size_y_, system_size_z_,
            pbc_x_, pbc_y_,pbc_z_)  < 1.0 ){
  ...
  double dist = systemMC_helper::distance_squared(positions_[i],
      ...
      system_size_y_, system_size_z_, pbc_x_, pbc_y_,pbc_z_);
